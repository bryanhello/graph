...
const drawAxis = (): void => {
    ...
    for (let x = 0; x < canvasWidth; x += 20) {
        ...
        const valueX = (x - canvasWidth / 2) / ratioConst;
        ctx.fillText(valueX.toString(), x, canvasHeight / 2 + 15);
    }
    ...
    for (let y = 0; y < canvasHeight; y += 20) {
        ...
        const valueY = (canvasHeight / 2 - y) / ratioConst;
        ctx.fillText(valueY.toString(), canvasWidth / 2 + 10, y);
    }
};
...
const drawFunc = (funcsToDraw: ((x: number) => number)[]): void => {
    ...
    for (let i: number = 0; i < funcsToDraw.length; i++) {
        ...
        for (let x: number = 0; x < canvasWidth; x = x + 0.1) {
            ...
            ctx.lineTo(x + 0.1, canvasHeight / 2 - func((y + x + 0.5)));
            ctx.stroke();
        }
    }
};
